...
- $S$ denotes the set of verified decentralized identities for all signatories.
...
Follow the key generation protocol described in section 3.1 of CGGMP20 [@cggmp20] to generate ECDSA secret shares with the following modifications:
...
2. At the beginning of Round 2, verify $\varphi _j$ from all $P_j$ by checking that the output of $Ver(pk_j, V_j, \varphi _j)$ is valid or report the culprit and halt.
...
Follow the signing protocol described in section 4.2 of CGGMP20 [@cggmp20] to generate an ECDSA signature with the following modifications:
...
2. At the end of Round 1, for each $P_i$ associated with the decentralized identity $I_i$ with address $pk_i$ and secret key $sk_i$, send 2 additional parameters to all $P_j$ as follows:
   - The decentralized identity address $pk_i$.
   - The signature $\varphi _i = Sig(sk_i, m)$.
3. At the beginning of the Output phase, verify $\varphi _j$ from all $P_j$ as follows:
   - Verify that $pk_i \in S_j$ or report the culprit and halt.
   - Verify $\varphi _i$ by checking that the output of $Ver(pk_i, m, \varphi _i)$ is valid or report the culprit and halt.

## 6. Key Refresh {#key-refresh}

Follow the key refresh protocol described in section 3.2 of CGGMP20 [@cggmp20] to generate new ECDSA secret shares with the following modifications:

1. After outputting the new secret share $x_i^\ast$, follow the share splitting protocol in [section 3.1](#share-splitting) to split $x_i^\ast$ into a new signing share $m_i^\ast$ and a sub-share $\beta _i^\ast$ for each party $P_i$.
2. Modify Stored State for each $P_i$ as follows:
   - Don't store $x_i^\ast$.
   - Replace $m_i$ with $m_i^\ast$ and $\beta _i$ with $\beta _i^\ast$.

## 7. Identity Authenticated Request Initiation and Verification {#decentralized-identity-request}
...
2. Compute the signature $\varphi = Sig(sk_i, t | C)$.
3. Broadcast $C$, $pk_i$, $t$ and $\varphi$.

### 7.2. Identity Authenticated Request Verification {#identity-authed-request-verification}

To verify an identity authenticated request with a command $C$ from a party $P_i$ given its associated decentralized identity address $pk_i$, a timestamp $t$, a signature $\varphi$ and a set of verified participant decentralized identities $M_j$ as input:
...
3. Verify $\varphi$ by checking that the output of $Ver(pk_i, t | C, \varphi)$ is valid or report the culprit and halt.

## 8. Identity Challenge {#identity-challenges}
...
To issue an identity challenge to a party $P_i$ from all verifying parties $P_j$:
1. Sample a random $v_j$. 
2. Broadcast $v_j$ to all parties, such that all parties can compute $v = \sum _j v_j$ where $j \neq i$.
...
1. Compute $v = \sum _j v_j$ where $j \neq i$. 
2. Compute the signature $\psi = Sig(sk_i, v)$. 
3. Broadcast $\psi$ to all parties.

#### 8.3. Identity Challenge Verification {#identity-challenge-verification}
...
1. Compute $v = \sum _j v_j$ where $j \neq i$.
2. Verify $\psi$ by checking that the output of $Ver(pk_i, v, \psi)$ is valid or report the culprit and halt.
...
3. For Pi, respond to the identity challenge by following the protocol in [section 8.2](#identity-challenge-response) with the following augmentations:
   - Generate an additional signature $\psi _i^ \ast = Sig(sk_i^ \ast, v)$.
...
4. For all $P_j$ where $j \neq i$:
   ...
   - Verify that $P_i$ controls the new decentralized identity address $pk_i^ \ast$ as follows:
     ...
     - Verify $\psi ^ \ast$ by checking that the output of $Ver(pk_i^ \ast, v, \psi ^ \ast)$ is valid or report the culprit and halt.
...
5. For $P_i$, upon receiving confirmation of successful rotation from a quorum of $P_j$:
   - Compute the new signing share $m_i^ \ast$ and sub-share $\beta _i^ \ast$ based on the new decentralized identity $I_i^ \ast$ as follows:
     ...
     - Follow the share splitting protocol in [section 3.1](#share-splitting) to split $x_i^ \ast$ into a new signing share $m_i^ \ast$ and a sub-share $\beta _i^ \ast$ based on the new decentralized identity $I_i^ \ast$.
   - Modify Stored State as follows:
     - Replace $pk_i$ with $pk_i^ \ast$.
     - Replace $m_i$ with $m_i^ \ast$.
     - Replace $\beta _i$ with $\beta _i^ \ast$.

## 10. Share Addition and Removal {#share-addition-and-removal}
...
### 10.1. Share Addition {#share-addition}

Share addition for a new party $P_i$ with associated decentralized identity $I_i$ proceeds as follows:

1. For all $P_j$ where $j \neq i$, initiate an identity challenge for $P_i$ by following the protocol in [section 7.1](#identity-authed-request-initiation).
2. For $P_i$, respond to the identity challenge by following the protocol in [section 7.2](#identity-authed-request-verification).
...
Therefore, threshold modification can be achieved by following the key refresh protocol described in section 3.2 of CGGMP20 [@cggmp20] and [section 6](#key-refresh) of this document, with some modifications based on the key generation protocols described in GG18 [@gg18] and GG20 [@gg20], and following the instructions in section 1.2.8 of CGGMP20 [@cggmp20].

In particular, this is achieved by performing a $t$-out-of-$n$ Feldman's VSS [@feldman-vss] sharing of the values $x_i$ (as defined in section 3.2 of CGGMP20 [@cggmp20]), with the new threshold $t$ used as the threshold parameter (similarly defined as $t$) for Feldman's VSS [@feldman-vss] protocol as described in section 2.8 and Phase 2 of section 3.1 in GG20 [@gg20] (and similary in section 2.6 and Phase 2 of section 4.1 in GG18 [@gg18]).

**NOTE:** Similar modifications can be applied to the signing protocol described in section 3.1 of CGGMP20 [@cggmp20] and [section 5](#signing) of this document to achieve a $t$-out-of-$n$ sharing of the secret key for $n \geq t+1$.
This is achieved by performing a $t$-out-of-$n$ Feldman's VSS [@feldman-vss] sharing of the value $x_i$ (as defined in section 3.1 of CGGMP20 [@cggmp20]), and following the instructions in section 1.2.8 of CGGMP20 [@cggmp20].

## 12. Share Recovery {#share-recovery}
...
### 12.1. Share recovery with a surviving quorum of honest parties {#share-recovery-quorum}
...
4. For all $P_j$ where $j \neq i$, verify the identity challenge response from $P_i$ by following the protocol in [section 8.3](#identity-challenge-verification). 
5. Follow the key refresh protocol described in [section 6](#key-refresh) if all verifications above pass.

### 12.2. Share recovery with a backup on user-controlled secondary or device-independent storage {#share-recovery-backup}

#### 12.2.1. Overview of share recovery with backup  {#share-recovery-backup-overview}
...
Therefore, a signing share $m$ and sub-share $\beta$ pair can be safely backed up to user-controlled secondary (e.g. a secondary device or a flash drive) or device-independent storage (e.g. Apple iCloud [^1], Google Drive [^2], Microsoft OneDrive [^3], Dropbox [^4] e.t.c) without exposing the secret share.

#### 12.2.2. Generating an encrypted backup for share recovery {#share-recovery-backup-encrypt}
...
Given a standardized phase $k$, a key derivation function $H$, a symmetric encryption algorithm $E$, this would proceed as follows:
...
#### 12.2.3. Decrypting an encrypted backup {#share-recovery-backup-decrypted}
...
Given a standardized phase $k$, a key derivation function $H$, a symmetric encryption algorithm $E$, the ciphertexts for the signing share $m_c$ and the ciphertext for the sub-share $\beta _c$, this would proceed as follows:
...
#### 12.2.4. Further security and usability considerations for share recovery {#share-recovery-backup-enhancements}
