...
\date{1st September, 2023 \textbar{} version 1.4}
...
\item
  ...
  \(Ver(pk_j, A \Vert V_j, \varphi _j)\) is valid or report the culprit
  and halt.
\item
...
\item
  ...
  \item
    ...
    \(Ver(pk_j, A \Vert V_j, \varphi _j)\) is valid or report the
    culprit and halt.
  \end{itemize}
\item
  After the Output phase, follow the share splitting protocol in
...
\item
  ...
  \item
    Verify \(\varphi _i\) by checking that the output of
    \(Ver(pk_i, A \Vert m, \varphi _i)\) is valid or report the culprit
    and halt.
  \end{itemize}
\end{enumerate}
...
\item
  Compute the signature \(\varphi = Sig(sk_i, A \Vert C \Vert t)\).
\item
...
\item
  ...
  \(Ver(pk_i, A \Vert C \Vert t, \varphi)\) is valid or report the
  culprit and halt.
\end{enumerate}
...
\item
  Compute \(v = \Vert _{j \neq i} \: v_j\).
\item
  Compute the signature \(\psi = Sig(sk_i, A \Vert v)\).
\item
...
\item
  ...
  \(Ver(pk_i, A \Vert v, \psi)\) is valid or report the culprit and
  halt.
\end{enumerate}
...
\item
  ...
  \item
    ...
    \(\psi _i^ \ast = Sig(sk_i^ \ast, A \Vert v)\).
  \item
...
\item
  ...
  \item
    ...
    \item
      ...
      \(Ver(pk_i^ \ast, A \Vert v, \psi ^ \ast)\) is valid or report the
      culprit and halt.
    \end{itemize}
  \item
...
\item
  ...
  \item
    ...
    \item
      Compute a signature
      \(\phi _j = Sig(sk_j, A \Vert v_j \Vert C \Vert t)\).
    \item
...
\item
  ...
  \item
    ...
    \(Ver(pk_c, A \Vert v_c \Vert C \Vert t, \phi _c)\) is valid or
    halt.
  \item
...
\item
  ...
  \item
    ...
    \item
      ...
      output of \(Ver(pk_c, A \Vert v_c \Vert C \Vert t, \phi _c)\) is
...
\textbf{NOTE:} Similar modifications can be applied to the key
generation protocol described in section 3.1 and figure 5 of CGGMP20
{[}\protect\hyperlink{ref-cggmp20}{1}{]} and
