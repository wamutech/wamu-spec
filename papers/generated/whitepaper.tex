...
Multisig wallets (e.g.~Safe \footnote{Safe.
  \url{https://safe.global/wallet}}) are already widely adopted
\footnote{Dune Analytics. {[}Mainnet{]} Safe.
...
controlled by multiple cryptographic identities, for mainstream users
...
wallet solutions that are controlled by multiple cryptographic
...
{[}\citeproc{ref-cggmp20}{1}{]}, GG20 {[}\citeproc{ref-gg20}{2}{]},
CMP20 {[}\citeproc{ref-cmp20}{3}{]} and FROST20
{[}\citeproc{ref-frost20}{4}{]}) that allow for asynchronous
...
{[}\citeproc{ref-cggmp20}{1}{]}) by cryptographically associating each
signing party with a cryptographic identity. This is achieved by:
...
\item
  ...
  \hyperref[share-splitting-and-reconstruction]{section 3}.
\item
...
\item
  ...
  \hyperref[share-recovery]{section 4}) that build on top of the above 2
  augmentations.
\end{itemize}
...
technical detail is provided for cryptographic identity authentication
and the rest of Wamu's sub-protocols. We refer the reader to Wamu's
technical specification {[}\citeproc{ref-wamu}{5}{]} for the technical
details that are not provided in this document.
...
associated cryptographic identity \(I\), the share splitting and
...
\item
  ...
  {[}\citeproc{ref-wiki:interpolation}{7}{]})
\item
...
\hyperref[share-splitting]{section 3.1} above) and access to the
cryptographic identity \(I\) with secret key \(sk\), the share
...
\item
  ...
  {[}\citeproc{ref-wiki:interpolation}{7}{]} using \(\alpha\) and
  \(\beta\) as inputs.
\item
...
cryptographic identity \(I_i\). The surviving quorum of honest parties
...
\(I_i\) matches a previously verified cryptographic identity for a
signatory.
...
\hyperref[share-splitting-and-reconstruction]{section 3} above, we note
that for any party \(P\), the combination of a signing share \(k\) and a
